...
class GameController {
  ...
  static async addVote(req, res) {
    ...
    try {
      const updatedGame = await GameService.addVote(+id, vote);
      if (!updatedGame) {
        return res.status(404).json(formatResponse(404, "Game not found"));
      }
      res
        .status(200)
        .json(formatResponse(200, "Vote added successfully", updatedGame));
    } catch ({ message }) {
      console.error(message);
      res
        .status(500)
  ...
  static async clearVoting(req, res) {
    const { id } = req.params;
    ...
    try {
      const updatedGame = await GameService.clearVoting(+id);
      if (!updatedGame) {
        return res.status(404).json(formatResponse(404, "Game not found"));
      }
      res
        .status(200)
        .json(formatResponse(200, "Voting cleared successfully", updatedGame));
    } catch ({ message }) {
      console.error(message);
      res
        .status(500)
