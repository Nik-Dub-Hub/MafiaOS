...
export const getAllPlayerThunk = createAsyncThunk<
  IServerResponse<PlayerArrayType>,
  void,
  { rejectValue: IServerResponse }
>(PLAYER_THUNK_TYPES.GET_ALL, async (_, { rejectWithValue }) => {
  try {
...
export const createPlayerThunk = createAsyncThunk<
  IServerResponse<IPlayer>,
  { game_id: number },
  { rejectValue: IServerResponse }
>(PLAYER_THUNK_TYPES.CREATE, async ({ game_id }, { rejectWithValue }) => {
  try {
    const { data } = await axiosInstance.post(PLAYER_ENDPOINT_PATH, {
      game_id,
    });
    return data;
  } catch (error) {
...
export const updatePlayerThunk = createAsyncThunk<
  IServerResponse<IPlayer>,
  { id: number; updateData: IPlayerForUpdate },
  { rejectValue: IServerResponse }
>(
...
export const deletePlayerThunk = createAsyncThunk<
  ...
  { id: number },
  { rejectValue: IServerResponse }
>(PLAYER_THUNK_TYPES.DELETE, async (id, { rejectWithValue }) => {
  try {
